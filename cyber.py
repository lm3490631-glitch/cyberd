...
# مفاتيح الـ API العالمية (تأكد من بقائها سرية)
VIRUSTOTAL_API_KEY = "e629c15676bcfbf170ec5b27d6854bacf231137123d81ebbd425c0048f955f34"
BREACH_API_KEY = "3b0bd76df3msh795a7db50ee4a2fp17d9abjsnb9da2a00c6d9"  # مفتاح فحص حقيقي مجاني
...
def get_real_email_breach(email):
    # استخدام سيرفر RapidAPI للاتصال بقاعدة بيانات BreachDirectory العالمية
            ...
            if re.match(r"[^@]+@[^@]+\.[^@]+", email_input):
                ...
                with st.spinner("جاري فحص وتتبع سجل الخروقات للبريد من الخوادم العالمية..."):
                    ...
                    if is_breached is None:
                        ...
                        # استخراج وعرض أسماء المصادر المخترقة حياً
